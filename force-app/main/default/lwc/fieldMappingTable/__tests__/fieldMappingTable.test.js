...
jest.mock(
  "@salesforce/apex/FieldMappingController.fetchProjects",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/FieldMappingController.fetchObjects",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
jest.mock(
  "@salesforce/apex/FieldMappingController.fetchFields",
  () => ({ default: jest.fn() }),
  { virtual: true }
);
  ...
  beforeEach(() => {
    ...
    fetchFields.mockImplementation(({ objectApiName }) => {
      if (objectApiName === "Account")
        return Promise.resolve(MOCK_ACCOUNT_FIELDS);
