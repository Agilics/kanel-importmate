...
export default class MainComponent extends LightningElement {
  ...
  get mappingSampleRows() {
    ...
    if (Array.isArray(rows) && rows.length) {
      return rows.slice(0, FM_SAMPLE_MAX);
    }
    return [];
  }
  ...
  async handleCreateProject() {
    ...
    try {
      const exists = await doesProjectExist({ name: this.projectName, targetObject: this.targetObject });
      if (exists) {
  ...
  get isMappingAndTransformation() {
    return this.currentProject && this.currentStep === STEPS.FIELD_MAPPING;
  }
...
handleStartMapping(event) {
  ...
  handleSidebarStepClick(event) {
    ...
    if (stepNumber === STEPS.PROJECT_SETUP || this.currentProject) {
      ...
      this.showToast(TOAST_VARIANTS.WARNING, MESSAGES.CREATE_PROJECT_FIRST, TOAST_VARIANTS.WARNING);
    }
  }
  ...
  handleContinueFromFieldMapper(event) {
    ...
    if (d.targetObjectApiName) this.mappingTargetObject = d.targetObjectApiName;
