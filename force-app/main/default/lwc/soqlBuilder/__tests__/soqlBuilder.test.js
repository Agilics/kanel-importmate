...
describe("c-soql-builder", () => {
  ...
  it("charge les objets et remplit le combobox", async () => {
    ...
    expect(combos[0].options.map((o) => o.value)).toEqual([
      "Account",
      "Contact"
    ]);
  });
  ...
  it("au changement d’objet, champs & ORDER BY", async () => {
    ...
    const orderByCombo =
      el.shadowRoot.querySelectorAll("lightning-combobox")[1];
    expect(orderByCombo.options[0].value).toBe("");
    expect(orderByCombo.options.slice(1).map((o) => o.value)).toEqual([
      "Name",
      "Website",
      "Parent.Name"
    ]);
  ...
  it("appelle Apex et construit datatable", async () => {
    ...
    combos[2].dispatchEvent(
      new CustomEvent("change", { detail: { value: "ASC" } })
    );
    combos[3].dispatchEvent(
      new CustomEvent("change", { detail: { value: "LAST" } })
    );
    await flushPromises();

    // LIMIT / OFFSET
    inputsAll[1].dispatchEvent(
      new CustomEvent("change", { detail: { value: 100 } })
    );
    inputsAll[2].dispatchEvent(
    ...
    expect(firstArg.params.fieldList).toEqual([
      "Name",
      "Website",
      "AnnualRevenue",
      "IsActive",
    ...
    expect(table.columns.map((c) => c.fieldName)).toEqual([
      "Id",
      "Name",
      "Website",
      "AnnualRevenue",
